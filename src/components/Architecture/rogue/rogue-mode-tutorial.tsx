  ...
  if (selectedSection) {
    return (
      <RogueFrame>
        ...
        {selectedSection !== 'squad-formation' && (
          <SectionInProgress sectionId={selectedSection} />
        )}
      </RogueFrame>
    );
  }
      ...
      <div style={{
        ...
        <div style={{
          ...
          {tutorialSections.map((section, index) => (
            <TutorialButton index={index} section={section} onClick={handleSectionSelect} />
          ))}
        </div>
      </div>
    </div>
...
const RogueFrame = ({children}: {children: React.ReactNode}) => {
  return <div style={{
    width: '100%',
    height: '100%',
    backgroundColor: 'white',
...
const TutorialButton = ({index, section, onClick}: {index: number, section: TutorialSection, onClick: (sectionId: string) => void}) => {
  return (
    <button
      key={section.id}
...
const SectionInProgress = ({sectionId}: {sectionId: string}) => {
  return (
    <div style={{
      position: 'absolute',
      top: '50%',
      ...
      <h2 style={{ color: '#4ecdc4', marginBottom: '20px' }}>
        {tutorialSections.find(s => s.id === sectionId)?.title}
      </h2>
      <p style={{ color: '#666' }}>
        此关卡正在开发中...
      </p>
